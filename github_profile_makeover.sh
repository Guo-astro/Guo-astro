...
read -r -d '' PROFILE_README_TEMPLATE <<'EOF'
...
readarray -t ALL_REPOS < <(gh repo list "$GITHUB_USER" --json name,stargazersCount,visibility --jq '.[] | @base64')

for encoded in "${ALL_REPOS[@]}"; do
  _jq() { echo "$encoded" | base64 --decode | jq -r "$1"; }
  name=$(_jq '.name')
  stars=$(_jq '.stargazersCount')
